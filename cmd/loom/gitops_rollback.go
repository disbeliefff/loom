...
func newGitOpsRollbackCommand(a *app) (*cobra.Command, error) {
	var (
		strategyName string
		serviceName  string
		dryRun       bool
	...
	cmd := &cobra.Command{
		Use:   "rollback",
		Short: "Rollback a service to its previous promotion state",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return a.runGitOpsRollback(strategyName, serviceName, dryRun)
		},
	}
...
func (a *app) runGitOpsRollback(strategyName, serviceName string, dryRun bool) error {
	...
	if vErr := promotion.Validate(strategy.Promotion); vErr != nil {
		return fmt.Errorf("validate promotion config: %w", vErr)
	}
	...
	commitMsg := fmt.Sprintf("rollback(%s): %s → %s", service.Key, result.OldTag, result.NewTag)
